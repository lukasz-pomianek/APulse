...
interface AppMetadataDao {
    ...
    fun insertMetadata(metadata: AppMetadata)
    
    @Update
    fun updateMetadata(metadata: AppMetadata)
    
    @Delete
    fun deleteMetadata(metadata: AppMetadata)
    
    @Query("DELETE FROM app_metadata WHERE requestId = :requestId")
    fun deleteMetadataForRequest(requestId: String)
